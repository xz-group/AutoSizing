...
class GCN_actor(nn.Module):
	...
	"""
	def __init__(self, in_dim_gcn, n_latent_var, out_dim):
		"""
		...
		self.layer_fc_1 = nn.Linear(4 * 9, n_latent_var)
		self.layer_fc_2 = nn.Linear(n_latent_var, out_dim * 7)


	def forward(self, obs_gcn, adj):
		"""
		...
		if shape_dim == 2:
			...
			activation_fc_1 = torch.tanh(self.layer_fc_1(activation_gcn_3))
			output = F.softmax(torch.reshape(self.layer_fc_2(activation_fc_1), (7, 3)), dim=1)

		else:
			activation_gcn_3 = torch.flatten(activation_gcn_3, start_dim=1)
			#activation_fc_2 = torch.cat((activation_gcn_3, activation_fc_1), dim=1)
			activation_fc_1 = torch.tanh(self.layer_fc_1(activation_gcn_3))
			output = F.softmax(torch.reshape(self.layer_fc_2(activation_fc_1), (dim_zero, 7, 3)), dim=2)
