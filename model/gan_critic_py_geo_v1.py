...
class GAT_critic(nn.Module):
    ...
    def __init__(self, nfeat, nhid, num_heads, nclass, dropout):
        ...
        self.gc3 = nn.Linear(nclass*7, 1)
            #FullyConnected(nclass*7, 1)
    ...
    def forward(self, obs, adj_matrix):
        ...
        activation2 = F.dropout(activation1, self.dropout, training=self.training)
        activation2 = self.gc2(activation2, adj_matrix, print_attn_probs=False)
        activation2 = F.elu(activation2)

        activation2 = torch.flatten(activation2, start_dim=1)
        activation3 = self.gc3(activation2)

        #shape = activation3.shape[0]
