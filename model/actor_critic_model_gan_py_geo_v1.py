...
from model.gan_actor_py_geo_v1 import GAT_actor
from model.gan_critic_py_geo_v1 import GAT_critic
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class ActorCritic(nn.Module):
    def __init__(self, state_dim, n_latent_var, num_heads, action_dim, dropout):

        super(ActorCritic, self).__init__()
        # actor
        self.action_layer = GAT_actor(state_dim, n_latent_var, num_heads, action_dim, dropout)
        # critic
        self.value_layer = GAT_critic(state_dim, n_latent_var, num_heads, action_dim, dropout)
