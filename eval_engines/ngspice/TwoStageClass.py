...
### WD: TwoStageClass defines various functions to extract the simulation results from NgSpice simulations.

class TwoStageClass(NgSpiceWrapper):
### WD: the path of TwoStageClass is defined in ngspice_vanilla_opamp.py     

### WD: where do this output_path come from? from NgSpiceWrapper?
### Yes, but how NgSpiceWrapper transfer this path into the TwoStageClass?
...
### WD: where does these ".csv" come from?
### WD: These ac.csv and dc.csv come from ngspice_wrapper.py simulation results
...
### WD: ac's simulation outputs have two parts: real and imagenary
...
### WD: InterpolatedUnivariateSpline from "import scipy.interpolate as interp"
        ...
        try:
            ...
            #     return xstart
            return xstop, False
### WD: sciopt.brentq finds a solution (zero) of a function fzero. Please see the following link:
...
### WD: TwoStageMeasManager defines various functions to for evaluation, such as _get_spec and compute_penalty.

### How do this class is recalled?
### It seems that this class repots real-time training details when runing val_autobag_ray.py code.
...
class TwoStageMeasManager(object):
    ...
    def compute_penalty(self, spec_nums, spec_kwrd):
        ...
        return penalties
### WD: calculate penalty functions here.
